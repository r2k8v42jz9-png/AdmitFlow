...
export function UniversityDetail({ university: u }: { university: University }) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ArrowLeft className="size-4" /> {t("ud.back")}
      </Link>
      ...
      <Card className="relative overflow-hidden">
        ...
        <div className="relative flex flex-col gap-6 p-6 sm:p-7 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div>
              ...
              <p className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
                ...
                {u.website && (
                  ...
                  >
                    <Globe2 className="size-4" /> {t("ud.visitWebsite")}
                  </a>
                )}
              </p>
          ...
          <div className="flex shrink-0 items-center gap-5 rounded-2xl border border-border/50 bg-background/40 p-4">
            <ScoreRing value={u.admissionProbability} size={104} stroke={9} label={t("ud.yourOdds")} gradientId={`odds-${u.id}`} />
            <div className="space-y-2">
              <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium", band.className)}>
                <TrendingUp className="size-3" /> {t(band.key)}
              </span>
              <div className="flex flex-col gap-2 pt-1">
                <Button variant="gradient" size="sm">
                  <Send className="size-4" /> {t("ud.startApplication")}
                </Button>
                <Button
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <QuickStat icon={Sparkles} label={t("ud.aiFit")} value={`${u.fitScore}`} tone="text-success" />
        <QuickStat icon={TrendingUp} label={t("ud.acceptance")} value={`${u.acceptanceRate}%`} />
        <QuickStat icon={Users} label={t("ud.students")} value={formatCompact(u.studentCount)} />
        <QuickStat icon={Globe2} label={t("ud.international")} value={`${u.intlPercent}%`} />
      </div>

      {/* Tabs */}
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="flex h-auto w-full flex-wrap justify-start gap-1 sm:w-auto">
          <TabsTrigger value="overview">{t("ud.tab.overview")}</TabsTrigger>
          <TabsTrigger value="programs">{t("ud.tab.programs")}</TabsTrigger>
          <TabsTrigger value="admissions">{t("ud.tab.admissions")}</TabsTrigger>
          <TabsTrigger value="tuition">{t("ud.tab.tuition")}</TabsTrigger>
          <TabsTrigger value="ai">{t("ud.tab.ai")}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview">
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="p-6 lg:col-span-2">
              <h3 className="font-display text-lg font-semibold">{t("ud.whyChoose", { name: u.shortName })}</h3>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 border-t border-border/60 pt-5 sm:grid-cols-4">
                <Stat label={t("ud.worldRank")} value={`#${u.rankWorld}`} />
                <Stat label={t("ud.nationalRank")} value={`#${u.rankNational}`} />
                <Stat label={t("ud.livingCost")} value={`${formatCurrency(u.livingCost, u.currency).replace(/\.00$/, "")}${t("ud.perYearShort")}`} />
                <Stat label={t("ud.tuition")} value={u.tuitionPerYear === 0 ? t("uni.free") : `${formatCurrency(u.tuitionPerYear, u.currency).replace(/\.00$/, "")}${t("ud.perYearShort")}`} />
              </div>
            </Card>

            <Card className="p-6">
              <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
                <CalendarClock className="size-4 text-primary" /> {t("ud.deadlines")}
              </h3>
              <ul className="mt-4 space-y-3">
                {u.deadlines.map((d) => {
              ...
              <div key={p.name} className="flex flex-col gap-2 p-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-3 pl-13 sm:pl-0">
                  ...
                  <span className="text-sm font-semibold tabular-nums">
                    {p.tuitionPerYear === 0 ? t("uni.free") : `${formatCurrency(p.tuitionPerYear, u.currency).replace(/\.00$/, "")}${t("ud.perYearShort")}`}
                  </span>
                </div>
              </div>
            ))}
        ...
        <TabsContent value="admissions">
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="p-6 lg:col-span-2">
              <h3 className="font-display text-lg font-semibold">{t("ud.requirements")}</h3>
              <p className="mt-1 text-sm text-muted-foreground">{t("ud.requirementsSub", { name: u.shortName })}</p>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 border-t border-border/60 pt-5 sm:grid-cols-4">
                <Stat label={t("ud.essays")} value={`${u.requirements.essays}`} />
                <Stat label={t("ud.recommendations")} value={`${u.requirements.recommendations}`} />
                <Stat label={t("ud.acceptanceRate")} value={`${u.acceptanceRate}%`} />
                <Stat label={t("ud.admitOdds")} value={`${u.admissionProbability}%`} />
              </div>
            </Card>

            <Card className="flex flex-col items-center p-6 text-center">
              <h3 className="font-display text-lg font-semibold">{t("ud.admissionProb")}</h3>
              <ScoreRing
              ...
              <span className={cn("inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium", band.className)}>
                <TrendingUp className="size-3.5" /> {t(band.key)}
              </span>
              <p className="mt-3 text-xs text-muted-foreground">{t("ud.probDesc")}</p>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="tuition">
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="p-6">
              <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
                <Wallet className="size-4 text-primary" /> {t("ud.annualCost")}
              </h3>
              <div className="mt-5 space-y-4">
                <CostRow label={t("ud.tuition")} value={u.tuitionPerYear === 0 ? t("uni.free") : formatCurrency(u.tuitionPerYear, u.currency).replace(/\.00$/, "")} />
                <CostRow label={t("ud.livingCost")} value={formatCurrency(u.livingCost, u.currency).replace(/\.00$/, "")} />
                <div className="flex items-center justify-between border-t border-border/60 pt-4">
                  <span className="text-sm font-medium">{t("ud.estTotal")}</span>
                  <span className="font-display text-lg font-bold">
                    {formatCurrency(u.tuitionPerYear + u.livingCost, u.currency).replace(/\.00$/, "")}
                    <span className="text-xs font-normal text-muted-foreground">{t("ud.perYearShort")}</span>
                  </span>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-6 lg:col-span-2">
              <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
                <Award className="size-4 text-primary" /> {t("ud.scholarships")}
              </h3>
                  ...
                  return (
                    <div key={s.name} className="rounded-xl border border-border/50 bg-background/30 p-4">
                      ...
                      <div className="mt-3 flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">{t("ud.applyBy", { date: formatDate(s.deadline) })}</span>
          ...
          <Card className="relative overflow-hidden p-6">
            ...
            <div className="relative flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-display text-lg font-semibold">{t("ud.aiInsight")}</h3>
            ...
            <div className="relative mt-6 flex flex-wrap gap-2 border-t border-border/60 pt-5">
              <Button variant="gradient" size="sm" asChild>
                <Link href="/mentor">
                  <Sparkles className="size-4" /> {t("cmd.ask")}
                </Link>
              </Button>
              <Button variant="outline" size="sm" asChild>
                <Link href="/roadmap">
                  <CalendarClock className="size-4" /> {t("ud.buildRoadmap")}
                </Link>
              </Button>
            </div>
          </Card>
    ...
    <div>
      <div className="mb-1.5 flex items-center justify-between text-sm">
        ...
        <span className="flex items-center gap-1.5">
          ...
          <span className="text-muted-foreground">{t("ud.typical")} {format(required)}</span>
          {hasValue &&
            (meets ? (
